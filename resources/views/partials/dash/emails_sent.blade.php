    ...
    <div class="col-md-6">
        <div class="{{ $settingsTask['column_class'] }}" style="overflow-x: auto;">
            <h3>{{ $settingsTask['chart_title'] }}</h3>
            <table class="table table-bordered table-striped">
                <thead>
                <tr>
                    @foreach($settingsTask['fields'] as $key => $value)
                        <th>
                            {{ trans(sprintf('cruds.%s.fields.%s', $settingsTask['translation_key'] ?? 'pleaseUpdateWidget', $key)) }}
                        </th>
                    @endforeach
                </tr>
                </thead>
                <tbody>
                @forelse($settingsTask['data'] as $entry)
                    <tr>
                        @foreach($settingsTask['fields'] as $key => $value)
                            <td>
                                @if($value === '')
                ...
                @empty
                    <tr>
                        <td colspan="{{ count($settingsTask['fields']) }}">{{ __('No entries found') }}</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
